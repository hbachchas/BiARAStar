...
inline qint64 findHeuristic_F(PUZZLENODE *current, searchEnv::Environment *env)
{
    ...
    // skipping the 0th element; ignoring manhatten distance of the blank
    for (int i=0; i-env->puzzleSize; i++)
        for (int j=0; j-env->puzzleSize; j++)
        {
            if (current->array[i][j])
            {
                ...
                sum += (xDiff + yDiff) * (qint64)(sqrt(current->array[i][j]) * SQRT_PRECISION);
            }
        }
    return sum;
}
...
inline int getMovedTileFaceValue(PUZZLENODE *current, PUZZLENODE *child)
{
    int sum = 0;
    for (int i=0; i<GPuzzleSize; i++)
        for (int j=0; j<GPuzzleSize; j++)
            sum += qAbs( current->array[i][j] - child->array[i][j] );
    return sum/2;
}
    ...
    */
    ...
    for (int timecount = 0; timecount<((qint64)((MAX_POSSIBLE_TIME_MACRO - TIME_INCREMENT_FACTOR) / TIME_INCREMENT_FACTOR)); timecount++)
    {
        ...
        if ( !signalUnfinishedSearch )
            out1 << " " << QString::number( (qint64)(printInfo[timecount].solutionCost / SQRT_PRECISION) ) << endl;
        else
            out1 << " #######" << endl;
        out1.flush();
        file.close();
    ...
    while(goal->fForward > openlist->at(0)->fForward)   // conflict
    {
        ...
        else        // set ptParent for each child
        {
            while(successors.size())
            {
                ...
                if ( child->gForward > current->gForward + (qint64)( sqrt(getMovedTileFaceValue(current, child)) * SQRT_PRECISION ) )       // cost to any child is 1 or sqrt(2)
                {
                    child->gForward = current->gForward + (qint64)( sqrt(getMovedTileFaceValue(current, child)) * SQRT_PRECISION );
        ...
        if ( numberOfNodesExpandedForward < numberOfNodesExpandedBackward )   // Expand Forward
        {
            ...
            else        // set ptParent for each child
            {
                while(successors.size())
                {
                    ...
                    if (!child->visitedForward)
                        child->gForward = MY_INFINITY;
                    if ( child->gForward > current->gForward + (qint64)( sqrt(getMovedTileFaceValue(current, child)) * SQRT_PRECISION ) )
                    {
                        child->gForward = current->gForward + (qint64)( sqrt(getMovedTileFaceValue(current, child)) * SQRT_PRECISION );
                        child->fForward = child->gForward + epsilon->float_p * findHeuristic_F(child, env);
                        child->parentForward = current;
        ...
        else        // Expand Backward
        {
            ...
            else        // set ptParent for each child
            {
                while(successors.size())
                {
                    ...
                    if ( child->gBackward > current->gBackward + (qint64)( sqrt(getMovedTileFaceValue(current, child)) * SQRT_PRECISION ) )
                    {
                        child->gBackward = current->gBackward + (qint64)( sqrt(getMovedTileFaceValue(current, child)) * SQRT_PRECISION );
